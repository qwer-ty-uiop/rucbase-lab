...
/**
 * @brief DiskManager takes care of the allocation and deallocation of pages
 * within a database. It performs the reading and writing of pages to and from
...
class DiskManager {
   ...
   private:
    ...
    // atomic原子操作：不需要用到互斥量加锁（无锁）技术的多线程并发编程方式，并且原子操作在多线程中不会被打断执行
    // 互斥量的加锁一般是针对一个代码段（几行代码),
    // 原子操作针对的都是一个变量，而不是针对一个代码段
    // Read ---> Modified ----> Write
};
