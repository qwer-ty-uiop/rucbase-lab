...
class SmManager {
   ...
   public:
    ...
    void rollback_delete(const std::string& tab_name, const RmRecord& rec, const Rid& rid, Context* context);

    void rollback_update(const std::string& tab_name, const Rid& rid,const RmRecord& rec, Context* context);
    
    // redo
    void recovery_insert(const std::string& tab_name, const Rid& rid, const RmRecord& rec, std::shared_ptr<Transaction> trans);

    void recovery_delete(const std::string& tab_name, const Rid& rid, std::shared_ptr<Transaction> trans);

    void recovery_update(const std::string& tab_name, const Rid& rid, const RmRecord& rec, std::shared_ptr<Transaction> trans);

    // undo
    void rollback_insert(const std::string& tab_name, const Rid& rid, std::shared_ptr<Transaction> trans);

    void rollback_delete(const std::string& tab_name, const RmRecord& rec, const Rid& rid, std::shared_ptr<Transaction> trans);

    void rollback_update(const std::string& tab_name, const Rid& rid,const RmRecord& rec, std::shared_ptr<Transaction> trans);
 
    // for debug
    void show_index(const std::string ix_name){
        if(ihs_.find(ix_name) != ihs_.end())
            ihs_[ix_name]->print_tree();
        else
            std::cout << "Index " << ix_name << " not found" << std::endl;
    }
};