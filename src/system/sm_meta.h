...
struct IndexMeta {
    ...
    int get_col_offset(const std::string &col_name) const {
        int offset = 0;
        for(auto& col: cols) {
            if(col.name == col_name) break;
            offset += col.len;
        }
        return offset;
    }
...
struct TabMeta {
    ...
    bool is_index(std::vector<std::string>& col_names){
        size_t max_len = 0;
        for(auto& index: indexes) {
            size_t i =0;
            for(;i<col_names.size();++i) 
                if(std::find(col_names.begin(),col_names.end(),index.cols[i].name)==col_names.end()) break;
            if(i>=1 && i>=max_len) {
                max_len = i;
                col_names.clear();
                for(auto& col: index.cols) col_names.push_back(col.name); // 调整索引顺序
                break;
            }
        }
        return max_len>=1;
