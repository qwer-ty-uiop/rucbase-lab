...
class InsertLogRecord : public LogRecord {
   public:
    ...
    InsertLogRecord(txn_id_t txn_id,
                    RmRecord& insert_value,
                    Rid& rid,
...
class DeleteLogRecord : public LogRecord {
   public:
    ...
    DeleteLogRecord(txn_id_t txn_id,
                    RmRecord& delete_value,
                    Rid& rid,
                    std::string table_name)
        : DeleteLogRecord() {
        log_tid_ = txn_id;
...
class LogBuffer {
   public:
    ...
    void write_log_record(LogRecord* log_record) {
...
class LogManager {
   public:
    LogManager(DiskManager* disk_manager) { 
        ...
        stop = false; 
        flag = false;
        flush_log_thread_ = std::thread(&LogManager::flush_log_periodically, this);
    }
    ~LogManager() {
        stop = true;
        flush_log_thread_.join();
        flush_log_to_disk();
    }
