...
enum class Operation { FIND = 0,
                       INSERT,
                       DELETE };  // 三种操作：查找、插入、删除
...
inline int ix_compare(const char* a, const char* b, ColType type, int col_len) {
    switch (type) {
        ...
        case TYPE_BIG_INT: {
            ...
            long long int ib = *(long long int*)b;
            return (ia < ib) ? -1 : ((ia > ib) ? 1 : 0);
        }
        case TYPE_FLOAT: {
            float fa = *(float*)a;
        ...
        case TYPE_STRING:
            // col_len = std::max(strlen(a), strlen(b));
...
class IxNodeHandle {
   ...
   public:
    ...
    void print_node() {
        std::fstream outfile;
        outfile.open("bplusT.txt", std::ios::out | std::ios::app);
        outfile << "pageid:" << page->get_page_id().page_no << "\n";
        outfile << "|";
        for (int i = 0; i < page_hdr->num_key; i++) {
            outfile << *(int*)(keys + i * file_hdr->col_tot_len_) << " |";
        }
        outfile << "\n####################\n";
        outfile.close();
    }
...
class IxIndexHandle {
   ...
   private:
    ...
    int fd_;                               // 存储B+树的文件
    std::unique_ptr<IxFileHdr> file_hdr_;  // 存了root_page，但其初始化为2（第0页存FILE_HDR_PAGE，第1页存LEAF_HEADER_PAGE）
    std::mutex root_latch_;

    std::shared_ptr<IxNodeHandle> node_;  // 当前正在插入的页
    std::shared_ptr<IxNodeHandle> head_;  // 页链表头
   ...
   public:
    ...
    void print_tree() {
        std::fstream outfile;
        outfile.open("bplusT.txt", std::ios::out);
        outfile << "fd:" << fd_ << "\n";
        std::queue<IxNodeHandle> bplus;
        std::shared_ptr<IxNodeHandle> root = fetch_node(file_hdr_->root_page_);
        outfile << "root:" << file_hdr_->root_page_ << "\n";
        outfile.close();
        bplus.push(*root);
        while (!bplus.empty()) {
            auto& it = bplus.front();
            if (!it.is_leaf_page()) {
                for (int i = 0; i < it.get_size(); i++) {
                    std::shared_ptr<IxNodeHandle> child = fetch_node(it.value_at(i));
                    bplus.push(*child);
                }
            }
            it.print_node();
    ...
    bool contains(const char* key, std::shared_ptr<Transaction> txn) {
        std::scoped_lock<std::mutex> lock(root_latch_);
        std::shared_ptr<IxNodeHandle> target = find_leaf_page(key, Operation::FIND, txn).first;
        bool ret = target->is_leaf(key);
        assert(buffer_pool_manager_->unpin_page(target->get_page_id(), false));
        return ret;
    }

    // for search
    bool get_value(const char* key, std::vector<Rid>* result, std::shared_ptr<Transaction> txn);

    std::pair<std::shared_ptr<IxNodeHandle>, bool> find_leaf_page(const char* key, Operation operation, std::shared_ptr<Transaction> txn, bool find_first = false);

    // for insert
    page_id_t insert_entry(const char* key, const Rid& value, std::shared_ptr<Transaction> txn);
    void sorted_insert(const char* key, const Rid& value, std::shared_ptr<Transaction> txn);

    std::shared_ptr<IxNodeHandle> split(IxNodeHandle* node);
    std::shared_ptr<IxNodeHandle> sorted_split(IxNodeHandle* node);

    void insert_into_parent(IxNodeHandle* old_node, const char* key, IxNodeHandle* new_node, std::shared_ptr<Transaction> txn);

    // for delete
    bool delete_entry(const char* key, std::shared_ptr<Transaction> txn);

    bool coalesce_or_redistribute(IxNodeHandle* node, std::shared_ptr<Transaction> txn = nullptr, bool* root_is_latched = nullptr);
    bool adjust_root(IxNodeHandle* old_root_node);

    void redistribute(IxNodeHandle* neighbor_node, IxNodeHandle* node, IxNodeHandle* parent, int index);

    bool coalesce(IxNodeHandle** neighbor_node, IxNodeHandle** node, IxNodeHandle** parent, int index, std::shared_ptr<Transaction> txn, bool* root_is_latched);
