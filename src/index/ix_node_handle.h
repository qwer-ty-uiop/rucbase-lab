...
static const bool binary_search =
    true;  // 控制在lower_bound/uppper_bound函数中是否使用二分查找
...
class IxNodeHandle {
   ...
   private:
    const IxFileHdr*
        file_hdr;  // 头部文件信息，用到了file_hdr的keys_size, col_len
    Page* page;    // 存储节点页面
   ...
   public:
    ...
    char* get_key(int key_idx) const {
        return keys + key_idx * file_hdr->col_len;
    }

    Rid* get_rid(int rid_idx) const { return &rids[rid_idx]; }

    void set_key(int key_idx, const char* key) {
        memcpy(keys + key_idx * file_hdr->col_len, key, file_hdr->col_len);
    }
