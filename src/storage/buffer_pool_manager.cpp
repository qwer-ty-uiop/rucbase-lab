...
/**
 * @brief 更新页面数据, 为脏页则需写入磁盘，更新page元数据(data, is_dirty,
 * page_id)和page table
 *
 * @param page 写回页指针
 ...
 */
void BufferPoolManager::UpdatePage(Page* page,
                                   PageId new_page_id,
...
bool BufferPoolManager::UnpinPage(PageId page_id, bool is_dirty) {
    ...
    // pin_count等于0了就调用replacer将他unpin
    if (unpin_page.pin_count_ <= 0) {
        if (is_dirty) {
            disk_manager_->write_page(page_id.fd, page_id.page_no,
                                      unpin_page.data_, PAGE_SIZE);
        }
        replacer_->Unpin(frame_id);
        page_table_.erase(page_id);
        free_list_.push_back(frame_id);
    }
    return unpin_page.pin_count_ >= 0 ? true : false;
}
...
bool BufferPoolManager::FlushPage(PageId page_id) {
    ...
    disk_manager_->write_page(page_id.fd, page_id.page_no, flush_page.data_,
                              PAGE_SIZE);
...
Page* BufferPoolManager::NewPage(PageId* page_id) {
    ...
    // 3.   Pick a victim page P from either the free list or the replacer.
...
bool BufferPoolManager::DeletePage(PageId page_id) {
    ...
    // 2.2  If P exists, but has a non-zero pin-count, return false. Someone is
    // using the page.
